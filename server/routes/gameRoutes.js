...
async function startTimer(duration) {
  ...
  const timerInterval = setInterval(() => {
    if (time <= 0) {
      ...
      console.log(
        `Player ${currentPlayer} forfeits. Winner is Player ${winner}`
      );
...
router.post("/joinGame", (req, res) => {
  ...
  console.log(
    `API Request: POST /joinGame, username:${username} | SessionID:${sessionID} `
  );
...
router.post("/startgame", (req, res) => {
  const { sessionID, username, gamemode, difficulty } = req.body;
  console.log("POST /startgame received");

  if (!sessionID || !username || !gamemode) {
    return res.status(400).json({ error: "Missing required fields" });
  }

  try {
    game.startPlayerVsAI(sessionID, username, gamemode, difficulty);
    return res.status(200).json({ message: "POST /startgame" });
  } catch (error) {
