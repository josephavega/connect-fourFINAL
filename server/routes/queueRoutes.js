...
router.post("/leaveQueue", (req, res) => {
  ...
  console.log(`${sessionID} requested to leave the queue.`);
  if (users.userExists(sessionID)) {
    ...
    return res
      .status(200)
      .json({ success: true, message: `${username} removed from Queue` });
  } else {
...
router.post("/joinQueue", (req, res) => {
  ...
  if (!sessionID || !username) {
    return res
      .status(400)
      .json({ success: false, error: "SessionID and username are required" });
  }
  ...
  if (users.userInQueue(sessionID)) {
    return res
      .status(409)
      .json({ success: false, message: "User is already in the queue" });
  }
...
router.get("/getUsername/:sessionID", (req, res) => {
  ...
  if (users.userInQueue(sessionID)) {
    ...
    if (user && user.username) {
      console.log(
        `Username found for sessionID ${sessionID}: ${user.username}`
      ); // Log to confirm username found
